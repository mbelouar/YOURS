...
<%
    ...
    // Vérifier l'authentification partenaire
    if (session == null || session.getAttribute("partner") == null) {
        // Rediriger vers login avec l'URL de redirection
        String redirectUrl = request.getRequestURI();
        if (request.getQueryString() != null) {
            redirectUrl += "?" + request.getQueryString();
        }
        response.sendRedirect(request.getContextPath() + "/pages/auth/login.jsp?redirect=" + 
            java.net.URLEncoder.encode(redirectUrl, "UTF-8"));
        return;
    }
    
    // Récupérer les données du partenaire depuis la session
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('profileForm').addEventListener('submit', function(e) {
        ...
        // Send data to server
        const url = '${pageContext.request.contextPath}/partner/update-profile';
        ...
        .then(data => {
            if (data.success) {
                showSuccessMessage(data.message || 'Profil mis à jour avec succès !');
                // Reload page after a short delay to refresh session data
                setTimeout(() => {
                    window.location.reload();
                }, 1500);
            } else {
                showErrorMessage(data.error || 'Erreur lors de la mise à jour du profil');
        ...
        .catch(error => {
            ...
            showErrorMessage('Erreur lors de la mise à jour du profil: ' + error.message);
            submitBtn.innerHTML = originalText;
            submitBtn.disabled = false;
        });
    });
...
function loadUserData() {
    try {
        // Charger les données depuis la session serveur (passées via JSP)
        // Échapper les apostrophes et backslashes pour JavaScript
        <%
        String prenom = partner.getPrenom() != null ? partner.getPrenom().replace("\\", "\\\\").replace("'", "\\'") : "";
        String nom = partner.getNom() != null ? partner.getNom().replace("\\", "\\\\").replace("'", "\\'") : "";
        String mail = partner.getMail() != null ? partner.getMail().replace("\\", "\\\\").replace("'", "\\'") : "";
        String numTelephone = partner.getNumTelephone() != null ? partner.getNumTelephone().replace("\\", "\\\\").replace("'", "\\'") : "";
        %>
        const partnerData = {
            prenom: '<%= prenom %>',
            nom: '<%= nom %>',
        ...
        console.log('Loaded partner data from session:', partnerData);
        
        // Populate form fields with partner data
        document.getElementById('prenom').value = partnerData.prenom || '';
        document.getElementById('nom').value = partnerData.nom || '';
        document.getElementById('email').value = partnerData.mail || '';
        document.getElementById('telephone').value = partnerData.numTelephone || '';
        
        // Update page title with partner's name
        if (partnerData.prenom && partnerData.nom) {
            document.title = `Profil de ${partnerData.prenom} ${partnerData.nom} - YOURS`;
        }
    } catch (error) {
...
// Cette fonction n'est plus nécessaire car on envoie directement au serveur
...
function changePassword() {
    ...
    fetch('${pageContext.request.contextPath}/partner/change-password', {
        method: 'POST',
        body: formData,
        credentials: 'include'  // Inclure les cookies de session
    })
    ...
    .then(data => {
        if (data.success) {
            ...
            showErrorMessage(data.error || 'Erreur lors du changement de mot de passe');
        }
